...
Парсер для Авито, который находит квартиры по ссылке и заданным параметрам. Затем с помощью дообученных VqA моделей определяет, какой ремонт в данной квартире. В конце вся информация о квартире(описание, цена, ссылка на объявление) сохраняется в Excel таблице.  
...
, 4)Для каждого отсеянного изображения получает вердикт от модели Renovations_model, 5)Усредняет ответы модели на 4-ом шаге и возвращает джля данного обновления самый частовстречаемый класс ремонта, 6)Вставляет тип ремонта в соответствующую ячейку таблицы.
